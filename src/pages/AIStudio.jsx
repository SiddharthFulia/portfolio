      ...
      {error && (
        <div className="p-4 bg-gray-800/60 border border-gray-700 rounded-xl">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-yellow-900/50 text-yellow-500 flex items-center justify-center shrink-0">!</div>
            <div>
              <p className="text-yellow-400 text-sm font-medium">{error.includes('429') || error.includes('rate') || error.includes('limit') || error.includes('quota') ? 'High demand right now' : 'Generation failed'}</p>
              <p className="text-gray-500 text-xs mt-0.5">{error.includes('loading') ? 'The AI model is warming up. Please try again in 30 seconds.' : error.includes('429') || error.includes('rate') || error.includes('quota') ? 'Too many requests. Please wait a moment and try again.' : error}</p>
            </div>
          </div>
        </div>
      )}

      {loading && <div className={`${P} h-80 w-full flex items-center justify-center`}><span className="text-gray-600 text-sm">Generating image... this may take 30-60s</span></div>}
...
const VisionAI = () => {
  ...
  return (
    <div className="space-y-5">
      <div className="flex gap-2">
        <Input value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="What do you want to know about this image?"
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-2">
        ...
        <Button size="small" type={mode === 'cloud' ? 'primary' : 'default'}
          onClick={() => setMode('cloud')} style={mode === 'cloud' ? { background: '#4285f4' } : {}}>Cloud Voice</Button>
      </div>
        ...
        <>
          ...
          {error && (
            <div className="p-4 bg-gray-800/60 border border-gray-700 rounded-xl">
              ...
              <p className="text-gray-500 text-xs mt-1">Google Cloud TTS requires billing setup. Use Browser Voice instead — it works instantly with no setup.</p>
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="max-w-5xl mx-auto px-6 pt-32 pb-24">
        ...
        <Tabs
          ...
          size="large"
          items={[
            { key: 'imagegen', label: <span><PictureOutlined /> Image Gen</span>, children: <ImageGen /> },
            { key: 'vision', label: <span><EyeOutlined /> Vision AI</span>, children: <VisionAI /> },
            { key: 'tts', label: <span><SoundOutlined /> Text to Speech</span>, children: <TTS /> },
            { key: 'summarize', label: <span><FileTextOutlined /> Summarizer</span>, children: <Summarizer /> },
          ]}
        />
      </div>
    </div>
