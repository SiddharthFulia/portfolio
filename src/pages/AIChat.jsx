...
import {
  ...
  getChatJobStatus, updateConversation, compactConversationApi,
} from '../api/ai'
...
const AIChat = () => {
  ...
  const onCompact = () => {
    ...
    Modal.confirm({
      ...
      content: (
        <div className="text-sm text-gray-300 space-y-2">
          ...
          <p className="text-[11px] text-amber-300">
            ⚡ Summary is generated by Groq Llama 3.1 — free and takes ~2 seconds.
          </p>
        </div>
      ),
      okText: 'Compact',
      ...
      onOk: async () => {
        setCompacting(true)
        const { data, error: err } = await compactConversationApi(chatId, { keepLastN: 4 })
        setCompacting(false)
        if (err) { antMessage.error(err); return }
        antMessage.success(`Compacted ${data?.compacted ?? 0} messages`)
        // Reload the conversation so the messages array reflects the
        // hidden-old + new-system-summary state.
