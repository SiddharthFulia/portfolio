...
import { DownloadOutlined, LinkOutlined, CustomerServiceOutlined, SoundOutlined, AudioOutlined } from '@ant-design/icons'
import JobDetailPage from '../components/JobDetailPage'
import { getAudioStatus } from '../api/ai'

function AudioOutput({ job }) {
  if (!job?.outputUrl) return null
  const kindIcon = job.kind === 'music'
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-gray-800 bg-gray-900/60 p-4 space-y-3">
      <div className="flex items-center gap-2">
        <span className="text-xl text-gray-300">{kindIcon}</span>
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        ...
        <a href={job.outputUrl} target="_blank" rel="noreferrer"
          className="inline-flex items-center gap-1.5 text-[11px] text-gray-500 hover:text-amber-400 transition-colors break-all">
